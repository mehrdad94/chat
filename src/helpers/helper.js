...
// create a queue for tracking unsent messages, ...
// TODO on error
// TODO for each user new queue
export const messageQueue = (() => {
  const queue = []
  const intervalTimer = 3000
  let interval
  let intervalCounter = 0
  const intervalMaxCount = 2
  ...
  const run = () => {
    if (interval || !queue.length) return

    const task = queue[0].task
    ...
    interval = setInterval(() => {
      ...
      if (intervalCounter > intervalMaxCount) {
        remove()
